...
} from "@kvalitetsit/hjemmebehandling/Models/QuestionnaireResponse";
...
export default class QuestionnaireResponseTable extends Component<
  Props,
  State
> {
  ...
  async getQuestionnaireResponses(
    ...
    const carePlanIds = careplans
      .map((careplan) => careplan.id)
      .filter((id: string | undefined): id is string => !!id);
    const questionnaireIds = careplans
      .flatMap((careplan) => careplan.questionnaires)
      .map((x) => x.id);

    let questionnaireResponses: QuestionnaireResponse[] =
      await this.questionnaireResponseService.GetQuestionnaireResponsesForMultipleCareplans(
        carePlanIds,
        questionnaireIds,
        page,
        this.state.pagesize
  ...
  async populateData(page: number): Promise<void> {
    try {
      ...
      let questionnaireResponses: QuestionnaireResponse[] =
        await this.getQuestionnaireResponses(page);
  ...
  renderPage(): JSX.Element {
    ...
    const allQuestionnaires = this.props.careplans?.flatMap(
      (careplan) => careplan.questionnaires
    );

    return (
      <>
        <IsEmptyCard
          jsxWhenEmpty="Ingen besvarelser fundet"
        ...
        >
          <TableContainer>
            <Table>
              ...
              <TableBody>
                {this.state.questionnaireResponses.map(
                  (questionnaireResponse) => {
                    const questionnaire = allQuestionnaires.find(
                      (x) => x.id === questionnaireResponse.questionnaireId
                    )!;
                    const careplan = this.props.careplans.find((x) =>
                      x.questionnaires.find((q) => q.id == questionnaire.id)
                    )!;
                    ...
                    return (
                      <>
                        <TableRow>
                          ...
                          <TableCell>
                            {this.dateHelper.DateToString(
                              questionnaireResponse.answeredTime!
                            )}
                          </TableCell>
                          <TableCell>
                            <Stack
                              direction="row"
                              spacing={1}
                              className="questionnaireStatus"
                            >
                              <Typography>
                                {this.GetStatusRepresentation(
