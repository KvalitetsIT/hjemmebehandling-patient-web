...
export default class AnsweredPage extends Component<{}, State> {
  ...
  renderPage(): JSX.Element {
    ...
    const allQuestionnaires = this.state.careplans?.flatMap(
      ...
      <IsEmptyCard list={careplans} jsxWhenEmpty="Ingen behandlingsplan fundet">
        <Grid container spacing={2}>
          <IsEmptyCard
            ...
            jsxWhenEmpty="Ingen spørgeskemaer fundet på behandlingsplan"
          >
            <Grid item xs={12} className="headline-wrapper">
              <Typography className="headline">
                Spørgeskemaer til besvarelse i dag
              </Typography>
            </Grid>
            <Grid item xs={12}>
              <IsEmptyCard
                list={this.state.answeredTodayList}
                jsxWhenEmpty="Du har ikke flere spørgeskemaer der skal besvares"
              >
                <ScrollableRow
                  cols={2.5}
                  jsxList={this.state.answeredTodayList!.map(
                    (questionnaire) => {
                      const carePLanIncludingTheQuestionnaire = careplans?.find(
                        (careplan) =>
                      ...
                      return (
                        <QuestionnaireAnswerCard
                          careplan={carePLanIncludingTheQuestionnaire}
            ...
            <Grid item xs={12} className="headline-wrapper">
              <Typography className="headline">
                Andre spørgeskemaer til besvarelse
              </Typography>
            </Grid>
            <Grid item xs={12}>
              <IsEmptyCard
                list={this.state.answeredOtherdayList}
                jsxWhenEmpty="Ingen spørgeskemaer"
              >
                <ScrollableRow
                  cols={2.5}
                  jsxList={this.state.answeredOtherdayList!.map(
                    (questionnaire) => (
                      <QuestionnaireAnswerCard
                        careplan={careplans?.find((careplan) =>
                          careplan.questionnaires.find(
                            (q) => q.id == questionnaire.id
                          )
                        )}
